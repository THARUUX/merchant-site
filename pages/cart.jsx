...
export default function CartPage() {
    ...
    let deliveryFee = 0 + Math.floor(weightTotal) * 0;
                            ...
                            <div>
                                ...
                                {!cartProducts?.length && (
                                    <div className='w-full h-full flex justify-center flex-col items-center mt-24 text-xl.'>
                                        Your cart is empty
                                        <Link href={'/products/'}  className='mt-5 shadow-md hover:shadow-lg transition cursor-pointer hover:bg-slate-700 btn bg-main text-white px-3 py-2 rounded-sm '>Shop Now</Link>
                                    </div>
                                )}
                                {products?.length > 0 && (
