        ...
        if (orderInfo.line_items && orderInfo.line_items.length > 0) {
            for (const item of orderInfo.line_items) {
                ...
                try {
                    ...
                    const productResponse = await axios.get('/api/products?title=' + itemName);
                    const productInfo = productResponse.data;
                    setProductInfo(productInfo);

                    if (productInfo) {
                        const currentStock = productInfo.stock - item.quantity;
                        const categoryToSend = productInfo.category || null; // Use provided category or null
                    ...
                    console.error('Error fetching or updating product information:', error);
                }
            }
        }
    } catch (error) {
